...
# ============================================================
#  AI-автоответчик в ЛС (работает только в личных сообщениях с ботом)
# ============================================================
class DMAutoResponderStates(StatesGroup):
    setting_system    = State()  # ждём новый системный промт (личность ИИ)
    adding_trigger    = State()  # шаг 1: ввод триггера(ов)
    adding_response   = State()  # шаг 2: ввод ответа на триггер
    deleting_trigger  = State()  # ждём триггер для удаления

# Режимы работы автоответчика в ЛС.
DM_AR_MODE_OFF      = 'off'        # выключен
DM_AR_MODE_KEYWORDS = 'keywords'   # по ключевым словам (подстроке)
DM_AR_MODE_AI       = 'ai'         # ИИ на все сообщения

DM_AR_MODE_LABELS = {
    DM_AR_MODE_OFF:      '🔕 Выключен',
    DM_AR_MODE_KEYWORDS: '🔑 По ключевым словам',
    DM_AR_MODE_AI:       '🤖 ИИ на все сообщения',
}

DM_AR_DEFAULT_SYSTEM_PROMPT = (
...
# Сколько последних пар user+assistant держать в контексте (по умолчанию).
DM_AR_HISTORY_PAIRS = 10
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        # ===== AI-автоответчик в ЛС (dm_autoresponder) =====
        # Настройки режима и личности ИИ для бота в личке с пользователем.
        # Режимы: 'off' | 'keywords' | 'ai'
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS dm_autoresponder (
                user_id        BIGINT PRIMARY KEY REFERENCES users(user_id) ON DELETE CASCADE,
        ...
        # Ключевые слова для режима "По ключевым словам" в ЛС.
        ...
        ''')
        await conn.execute(
            'CREATE INDEX IF NOT EXISTS idx_dm_keywords_user '
            'ON dm_keywords (user_id)'
        )
...
# ============================================================
#  AI-автоответчик в ЛС — хелперы БД
# ============================================================
# Всё per-user:
#   dm_autoresponder: mode, system_prompt, history
#   dm_keywords:      триггеры и ответы (режим "по ключевым словам")

async def dm_ar_get_settings(user_id: int) -> Dict[str, Any]:
    """Получить настройки AI-автоответчика. Создаёт запись при первом обращении."""
    async with db_pool.acquire() as conn:
        # Гарантируем, что пользователь существует в users (FK на dm_autoresponder).
        # Идемпотентно, username/first_name не нужны.
        await conn.execute(
            'INSERT INTO users (user_id) VALUES ($1) '
            'ON CONFLICT (user_id) DO NOTHING',
            user_id,
        )
        # INSERT ... ON CONFLICT — атомарно создаём, если ещё нет
        await conn.execute(
            'INSERT INTO dm_autoresponder (user_id) VALUES ($1) '
            'ON CONFLICT (user_id) DO NOTHING',
            user_id,
        )
        row = await conn.fetchrow(
            'SELECT * FROM dm_autoresponder WHERE user_id = $1',
            user_id,
        )
        if not row:
            return {
        ...
        if isinstance(history, str):
            # на всякий случай — если где-то сохранили как JSON-строку
            try:
        ...
        return {
            'user_id': row['user_id'],
            'mode': row['mode'] or DM_AR_MODE_OFF,
            'system_prompt': row['system_prompt'] or '',
            'history': history or [],
        }


async def dm_ar_set_mode(user_id: int, mode: str) -> None:
    if mode not in DM_AR_MODE_LABELS:
        return
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO dm_autoresponder (user_id, mode) VALUES ($1, $2) '
            'ON CONFLICT (user_id) DO UPDATE '
...
async def dm_ar_set_system_prompt(user_id: int, prompt: str) -> None:
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO dm_autoresponder (user_id, system_prompt) '
            'VALUES ($1, $2) '
            'ON CONFLICT (user_id) DO UPDATE '
...
async def dm_ar_reset_system_prompt(user_id: int) -> None:
    """Сбросить к дефолтному системному промту (просто очищаем)."""
    await dm_ar_set_system_prompt(user_id, '')


async def dm_ar_get_history(user_id: int) -> List[Dict[str, str]]:
    s = await dm_ar_get_settings(user_id)
    return list(s.get('history') or [])


async def dm_ar_push_history(
    user_id: int, role: str, content: str,
) -> List[Dict[str, str]]:
    """Добавить сообщение в историю и обрезать до DM_AR_HISTORY_PAIRS*2."""
    history = await dm_ar_get_history(user_id)
    history.append({'role': role, 'content': content})
    history = history[-(DM_AR_HISTORY_PAIRS * 2):]
    async with db_pool.acquire() as conn:
        # pg jsonb — приводим Python-список к строке и кастуем в jsonb
        await conn.execute(
            'INSERT INTO dm_autoresponder (user_id, history) '
            'VALUES ($1, $2::jsonb) '
            'ON CONFLICT (user_id) DO UPDATE '
...
async def dm_ar_reset_history(user_id: int) -> None:
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO dm_autoresponder (user_id, history) '
            'VALUES ($1, \'[]\'::jsonb) '
...
async def dm_ar_add_keyword(
    user_id: int, trigger: str, response: str,
) -> None:
    """Добавить/перезаписать ключевое слово (case-insensitive)."""
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO dm_keywords (user_id, trigger, response) '
            'VALUES ($1, $2, $3) '
            'ON CONFLICT (user_id, trigger) DO UPDATE '
            'SET response = EXCLUDED.response, created_at = NOW()',
            user_id, trigger.lower(), response,
        )


async def dm_ar_delete_keyword(user_id: int, trigger: str) -> bool:
    async with db_pool.acquire() as conn:
        result = await conn.execute(
            'DELETE FROM dm_keywords WHERE user_id = $1 AND trigger = $2',
            user_id, trigger.lower(),
        )
        try:
...
async def dm_ar_list_keywords(user_id: int) -> List[Dict[str, Any]]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            'SELECT * FROM dm_keywords WHERE user_id = $1 ORDER BY id',
            user_id,
        )
        return [dict(r) for r in rows]


async def dm_ar_find_keyword_response(
    user_id: int, text: str,
) -> Optional[str]:
    """Вернуть первый подходящий ответ (подстрочное вхождение)."""
    kws = await dm_ar_list_keywords(user_id)
    if not kws:
        return None
    t = text.lower()
    for kw in kws:
...
def get_functions_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(
        text="AI-автоответчик в ЛС",
        callback_data="dm_autoresponder",
        style='primary',
        icon_custom_emoji_id=get_icon("CHAT")
    ))
    builder.row(InlineKeyboardButton(
...
# ============================================================
#  AI-автоответчик в ЛС
#  ────────────────────
#  Работает ТОЛЬКО в личных сообщениях с ботом.
#  Два режима:
#    • «По ключевым словам»  — отвечает заготовкой, если в тексте есть триггер
#    • «ИИ на все сообщения» — каждое входящее ЛС уходит в LLM с заданной личностью
#  Личность ИИ настраивается через системный промт.
# ============================================================

def _dm_ar_home_text(settings: Dict[str, Any],
                     kw_count: int, hist_len: int) -> str:
    sysp = (settings.get('system_prompt') or '').strip() \
        or DM_AR_DEFAULT_SYSTEM_PROMPT
    mode = settings.get('mode') or DM_AR_MODE_OFF
    return (
        f"{emoji('CHAT')} <b>AI-автоответчик в ЛС</b>\n\n"
        f"Режим: <b>{DM_AR_MODE_LABELS.get(mode, mode)}</b>\n"
        f"Ключевых слов: <b>{kw_count}</b>\n"
        f"Сообщений в контексте ИИ: <b>{hist_len}</b>\n\n"
        f"<b>Текущая личность ИИ:</b>\n"
        f"<code>{escape(sysp[:700])}</code>"
    )


def _dm_ar_main_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text="⚙️ Режим",
        callback_data="dm_ar:mode",
        style='primary',
        icon_custom_emoji_id=get_icon("GEAR"),
    ))
    builder.row(InlineKeyboardButton(
        text="🧠 Личность ИИ (системный промт)",
        callback_data="dm_ar:system",
        style='primary',
        icon_custom_emoji_id=get_icon("AI"),
    ))
    builder.row(InlineKeyboardButton(
        text="🔑 Ключевые слова",
        callback_data="dm_ar:keywords",
        style='primary',
        icon_custom_emoji_id=get_icon("BELL"),
    ))
    builder.row(InlineKeyboardButton(
        text="🧹 Сбросить историю ИИ",
        callback_data="dm_ar:reset",
        style='default',
        icon_custom_emoji_id=get_icon("SWEEP"),
    ))
    builder.row(InlineKeyboardButton(
...
def _dm_ar_mode_keyboard(current: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for value, label in DM_AR_MODE_LABELS.items():
        mark = " ✅" if value == current else ""
        builder.row(InlineKeyboardButton(
            text=label + mark,
            callback_data=f"dm_ar_mode:{value}",
            style='primary' if value == current else 'default',
            icon_custom_emoji_id=get_icon("CHECK") if value == current else get_icon("RIGHT"),
        ))
    builder.row(InlineKeyboardButton(
        text="↩️ Назад",
        callback_data="dm_autoresponder",
...
# --- Вход в функционал из главного меню ---
@dp.callback_query(F.data == "dm_autoresponder")
async def cb_dm_autoresponder(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    user_id = callback.from_user.id
    settings = await dm_ar_get_settings(user_id)
    kws = await dm_ar_list_keywords(user_id)
    text = _dm_ar_home_text(settings, len(kws), len(settings['history']))
    try:
        await callback.message.edit_text(text, reply_markup=_dm_ar_main_keyboard())
    except Exception:
        await callback.message.answer(text, reply_markup=_dm_ar_main_keyboard())
    await callback.answer()


# --- Переключение режима ---
async def _rerender_dm_ar_home(callback: CallbackQuery) -> None:
    user_id = callback.from_user.id
    settings = await dm_ar_get_settings(user_id)
    kws = await dm_ar_list_keywords(user_id)
    text = _dm_ar_home_text(settings, len(kws), len(settings['history']))
...
@dp.callback_query(F.data.startswith("dm_ar_mode:"))
async def cb_dm_ar_mode(callback: CallbackQuery):
    new_mode = callback.data.split(":", 1)[1]
    if new_mode not in DM_AR_MODE_LABELS:
        await callback.answer("Неизвестный режим")
        return
    await dm_ar_set_mode(callback.from_user.id, new_mode)
    await callback.answer(f"Режим: {DM_AR_MODE_LABELS[new_mode]}")
    await _rerender_dm_ar_home(callback)


# --- Экран «Режим» ---
@dp.callback_query(F.data == "dm_ar:mode")
async def cb_dm_ar_mode_screen(callback: CallbackQuery):
    settings = await dm_ar_get_settings(callback.from_user.id)
    text = (
        f"{emoji('GEAR')} <b>Режим AI-автоответчика в ЛС</b>\n\n"
        "• 🔕 <b>Выключен</b> — бот не отвечает на входящие ЛС-сообщения\n"
        "• 🔑 <b>По ключевым словам</b> — отвечает заготовкой, если в тексте есть триггер\n"
        "• 🤖 <b>ИИ</b> — отвечает на <i>каждое</i> сообщение через LLM с твоей личностью"
    )
    try:
        await callback.message.edit_text(
            text, reply_markup=_dm_ar_mode_keyboard(settings['mode'])
        )
    except Exception:
        await callback.message.answer(
            text, reply_markup=_dm_ar_mode_keyboard(settings['mode'])
...
# --- Экран «Личность ИИ» — ссылка на /dmsys ---
@dp.callback_query(F.data == "dm_ar:system")
async def cb_dm_ar_system(callback: CallbackQuery):
    await callback.message.answer(
        f"Чтобы изменить личность ИИ — отправь команду /dmsys "
        f"и следуй инструкциям в ответе."
    )
    await callback.answer()


# --- Экран «Ключевые слова» — ссылка на команды ---
@dp.callback_query(F.data == "dm_ar:keywords")
async def cb_dm_ar_keywords(callback: CallbackQuery):
    user_id = callback.from_user.id
    kws = await dm_ar_list_keywords(user_id)
    if not kws:
        await callback.message.answer(
            f"🔑 <b>Ключевых слов пока нет.</b>\n\n"
            f"Команды:\n"
            f"/dmadd — добавить\n"
            f"/dmlist — посмотреть\n"
            f"/dmdel — удалить"
        ...
        lines = ["🔑 <b>Твои ключевые слова:</b>\n"]
        for i, it in enumerate(kws, 1):
            tr = escape(it['trigger'])
            resp = escape(it['response'])
            if len(resp) > 80:
                resp = resp[:77] + "..."
            lines.append(f"{i}. <code>{tr}</code> → {resp}")
        lines.append("\nКоманды: /dmadd, /dmlist, /dmdel")
        await callback.message.answer("\n".join(lines))
    await callback.answer()


# --- Сброс истории ---
@dp.callback_query(F.data == "dm_ar:reset")
async def cb_dm_ar_reset(callback: CallbackQuery):
    await dm_ar_reset_history(callback.from_user.id)
    await callback.answer("🧹 История ИИ очищена")


# === Команды (для удобства) ===
@dp.message(Command("dmmode"))
async def cmd_dm_ar_mode(message: Message, state: FSMContext):
    await state.clear()
    settings = await dm_ar_get_settings(message.from_user.id)
    await message.answer(
        f"{emoji('GEAR')} <b>Режим AI-автоответчика в ЛС</b>\n\n"
        "• 🔕 <b>Выключен</b> — бот не отвечает на входящие ЛС-сообщения\n"
        "• 🔑 <b>По ключевым словам</b> — отвечает заготовкой, если в тексте есть триггер\n"
        "• 🤖 <b>ИИ</b> — отвечает на <i>каждое</i> сообщение через LLM с твоей личностью",
        reply_markup=_dm_ar_mode_keyboard(settings['mode']),
    )


@dp.message(Command("dmsys"))
async def cmd_dm_ar_system(message: Message, state: FSMContext):
    settings = await dm_ar_get_settings(message.from_user.id)
    cur = (settings.get('system_prompt') or '').strip() \
        or DM_AR_DEFAULT_SYSTEM_PROMPT
    await state.set_state(DMAutoResponderStates.setting_system)
    await message.answer(
        f"🧠 <b>Системный промт = личность ИИ</b>\n\n"
        f"<b>Текущий:</b>\n<code>{escape(cur)}</code>\n\n"
        f"Отправь новый промт одним сообщением.\n"
        f"Чтобы вернуть стандартную личность — пришли <code>default</code>.\n"
        f"Чтобы отменить — /cancel.\n\n"
        f"<b>Пример:</b>\n"
        f"<code>Ты — пират по имени Билли. Говоришь с \"йо-хо-хо!\", "
        f"слегка хулиганишь, но помогаешь по делу.</code>"
    )


@dp.message(DMAutoResponderStates.setting_system)
async def process_dm_ar_system(message: Message, state: FSMContext):
    if message.text is None:
        await message.answer("Пришли промт текстом. /cancel — отмена.")
        return
    text = message.text.strip()
    if not text:
        await message.answer("Промт не должен быть пустым. Попробуй ещё раз или /cancel.")
        return
    if text.lower() == "default":
        await dm_ar_reset_system_prompt(message.from_user.id)
        await state.clear()
        await message.answer(
            f"✅ Личность ИИ сброшена к стандартной.",
            reply_markup=_dm_ar_main_keyboard(),
        )
        return
    await dm_ar_set_system_prompt(message.from_user.id, text)
    await state.clear()
    await message.answer(
        f"✅ Личность ИИ обновлена!",
        reply_markup=_dm_ar_main_keyboard(),
    )


@dp.message(Command("dmadd"))
async def cmd_dm_ar_add(message: Message, state: FSMContext):
    await state.set_state(DMAutoResponderStates.adding_trigger)
    await message.answer(
        f"🔑 <b>Добавление ключевого слова</b>\n\n"
        f"<b>Шаг 1/2.</b> Пришли триггер(ы) — слово или фразу, на которые я среагирую.\n"
        f"Поиск регистронезависимый, по вхождению подстроки.\n"
        f"Можно сразу несколько — через запятую или с новой строки.\n\n"
        f"Пример: <code>привет, здравствуйте, хай</code>"
    )


@dp.message(DMAutoResponderStates.adding_trigger)
async def process_dm_ar_add_trigger(message: Message, state: FSMContext):
    if message.text is None:
        await message.answer("Пришли триггер текстом. /cancel — отмена.")
        return
    raw = message.text.strip()
    if not raw:
        await message.answer("Пусто. Пришли триггер, например <code>привет</code>.")
        return
    triggers = [t.strip().lower() for t in re.split(r"[,\n;]", raw) if t.strip()]
    if not triggers:
        await message.answer("Не получилось выделить триггеры. Попробуй ещё раз или /cancel.")
        return
    await state.update_data(triggers=triggers)
    await state.set_state(DMAutoResponderStates.adding_response)
    await message.answer(
        f"Триггеры: <b>{', '.join(escape(t) for t in triggers)}</b>\n\n"
        f"<b>Шаг 2/2.</b> Теперь пришли <b>текст ответа</b>, "
        f"который я отправлю в ЛС."
    )


@dp.message(DMAutoResponderStates.adding_response)
async def process_dm_ar_add_response(message: Message, state: FSMContext):
    if message.text is None:
        await message.answer("Пришли ответ текстом. /cancel — отмена.")
        return
    response = message.text.strip()
    if not response:
        await message.answer("Ответ не должен быть пустым. Попробуй ещё раз или /cancel.")
        return
    if len(response) > 4000:
        await message.answer(
            f"⚠️ Ответ слишком длинный (Telegram ограничивает сообщение 4096 символами). "
            f"Сократи и пришли ещё раз."
        )
        return
    data = await state.get_data()
    triggers: List[str] = data.get("triggers", [])
    for t in triggers:
        await dm_ar_add_keyword(message.from_user.id, t, response)
    await state.clear()
    await message.answer(
        f"✅ Готово! Добавлено ключевых слов: <b>{len(triggers)}</b>\n"
        f"Триггеры: {', '.join(escape(t) for t in triggers)}",
        reply_markup=_dm_ar_main_keyboard(),
    )


@dp.message(Command("dmlist"))
async def cmd_dm_ar_list(message: Message, state: FSMContext):
    await state.clear()
    kws = await dm_ar_list_keywords(message.from_user.id)
    if not kws:
        await message.answer(
            f"🔑 Список пуст. Добавь ключевые слова через /dmadd.",
            reply_markup=_dm_ar_main_keyboard(),
        )
        return
    lines = ["🔑 <b>Твои ключевые слова:</b>\n"]
    for i, it in enumerate(kws, 1):
        ...
        if len(resp) > 80:
            resp = resp[:77] + "..."
        lines.append(f"{i}. <code>{tr}</code> → {resp}")
    await message.answer("\n".join(lines), reply_markup=_dm_ar_main_keyboard())


@dp.message(Command("dmdel"))
async def cmd_dm_ar_del(message: Message, state: FSMContext):
    kws = await dm_ar_list_keywords(message.from_user.id)
    if not kws:
        await message.answer(
            f"🔑 Список пуст — удалять нечего.",
            reply_markup=_dm_ar_main_keyboard(),
        )
        return
    await state.set_state(DMAutoResponderStates.deleting_trigger)
    lines = [
        f"🗑 <b>Удаление ключевого слова</b>\n",
        f"Пришли триггер, который нужно удалить:\n",
    ]
    for i, it in enumerate(kws, 1):
        lines.append(f"{i}. <code>{escape(it['trigger'])}</code>")
    await message.answer("\n".join(lines))


@dp.message(DMAutoResponderStates.deleting_trigger)
async def process_dm_ar_del(message: Message, state: FSMContext):
    if message.text is None:
        await message.answer("Пришли триггер текстом. /cancel — отмена.")
        return
    trig = message.text.strip().lower()
    if not trig:
        await message.answer("Пришли триггер, или /cancel для отмены.")
        return
    ok = await dm_ar_delete_keyword(message.from_user.id, trig)
    await state.clear()
    if ok:
        await message.answer(
            f"🗑 Удалено: <code>{escape(trig)}</code>",
            reply_markup=_dm_ar_main_keyboard(),
        )
    else:
        await message.answer(
            f"Такого триггера нет: <code>{escape(trig)}</code>",
            reply_markup=_dm_ar_main_keyboard(),
        )


@dp.message(Command("dmreset"))
async def cmd_dm_ar_reset(message: Message, state: FSMContext):
    await state.clear()
    await dm_ar_reset_history(message.from_user.id)
    await message.answer(
        f"🧹 История диалога с ИИ очищена.",
        reply_markup=_dm_ar_main_keyboard(),
    )


@dp.message(Command("dmstatus"))
async def cmd_dm_ar_status(message: Message, state: FSMContext):
    await state.clear()
    settings = await dm_ar_get_settings(message.from_user.id)
    kws = await dm_ar_list_keywords(message.from_user.id)
    await message.answer(
        _dm_ar_home_text(settings, len(kws), len(settings['history'])),
        reply_markup=_dm_ar_main_keyboard(),
    )


# /cancel для DM-AR — сбрасываем наш FSM, не ломая общий /cancel
@dp.message(Command("cancel"))
async def cmd_cancel_dm_ar(message: Message, state: FSMContext):
    current = await state.get_state()
    if current in {
        DMAutoResponderStates.setting_system.state,
        DMAutoResponderStates.adding_trigger.state,
        DMAutoResponderStates.adding_response.state,
        DMAutoResponderStates.deleting_trigger.state,
    }:
        await state.clear()
        await message.answer(
            f"Ок, отменил.",
    ...
    # иначе — это чужой /cancel (для других фич), оставляем как есть
    # (другие хендлеры /cancel в проекте не зарегистрированы, поэтому
    # если состояние чужое — пользователь увидит стандартное «Ок, отменил»)


# === Catch-all: AI-автоответчик в ЛС ===
# Срабатывает только в личных сообщениях с ботом.
# В группах/каналах не реагирует. Команды обрабатываются отдельными хендлерами
# раньше (Command-фильтр специфичнее, и фильтры зарегистрированы до этого хендлера).
@dp.message(F.chat.type == "private")
async def handle_dm_autoresponder(message: Message, state: FSMContext):
    # Если активен любой FSM-шаг — пусть его обработает соответствующий хендлер
    if await state.get_state() is not None:
        return
    # Не обрабатываем свои/чужие бот-сообщения
    if not message.from_user or message.from_user.is_bot:
        return
    # Без текста (стикеры/фото/голос) — пока не отвечаем
    if not message.text or not message.text.strip():
        return
    # Команды уже отработаны Command-хендлерами; на всякий случай —
    # игнорируем, чтобы не закольцеваться
    ...
    settings = await dm_ar_get_settings(user_id)
    mode = settings.get('mode') or DM_AR_MODE_OFF
    if mode == DM_AR_MODE_OFF:
        return

    text = message.text.strip()

    if mode == DM_AR_MODE_KEYWORDS:
        resp = await dm_ar_find_keyword_response(user_id, text)
        if resp:
            try:
                await message.answer(resp)
            except Exception as e:
                logger.exception("dm_ar keyword send failed: %s", e)
        return

    if mode == DM_AR_MODE_AI:
        ...
        try:
            ...
            pass
        try:
            # Системный промт = личность ИИ (или дефолт)
            system_prompt = (
            ...
            if not answer:
                answer = "🤖 (ИИ вернул пустой ответ)"
            # Сохраняем в историю
            await dm_ar_push_history(user_id, 'user', text)
            await dm_ar_push_history(user_id, 'assistant', answer)
            ...
            logger.exception("dm_ar AI reply failed: %s", e)
            try:
                await message.answer(
                    f"⚠️ Не получилось получить ответ ИИ: {escape(str(e))}"
                )
            except Exception:
                pass
