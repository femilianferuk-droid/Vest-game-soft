...
async def ai_generator_prompt(message: Message, state: FSMContext):
    ...
    thinking = await message.answer(
        ...
        f"Модель: <code>{escape(LLM_MODEL)}</code>\n"
        f"Запрос: <i>{escape(user_prompt[:200])}</i>"
    )
    ...
    preview_lines = [
        f"{emoji('AI')} <b>3 варианта готовы. Выберите лучший:</b>",
        f"{emoji('INFO')} Запрос: <i>{escape(user_prompt[:160])}</i>",
        "",
    ]
    for i, v in enumerate(variants, 1):
        preview_lines.append(
            f"<b>Вариант {i}.</b> {escape(v['title'])}"
        )
        body = v['text']
        if len(body) > 220:
...
async def ai_generator_pick(callback: CallbackQuery, state: FSMContext):
    ...
    # Отдаём выбранный вариант отдельным сообщением, без лимита Telegram.
    # Если текст длиннее 4096 — режем на части.
    parts: List[str] = []
    remaining = body
    ...
    header = f"{emoji('SPARK')} <b>Вариант {idx + 1}. {title}</b>"
    first = f"{header}\n\n{escape(parts[0])}"
    await callback.message.answer(first)

    for extra in parts[1:]:
        await callback.message.answer(escape(extra))

    # Кнопка «Отправить в ЛС» — даём шорткат: пользователь может
    # скопировать текст кнопкой ниже.
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text=f"📋 Скопировать вариант {idx + 1}",
    ...
    await callback.message.answer(
        f"{emoji('CHECK')} <b>Готово.</b> Текст выше — без HTML-разметки "
        f"(экранирован), чтобы можно было копировать без мусора.",
...
async def ai_generator_show(callback: CallbackQuery, state: FSMContext):
    """Прислать чистый текст варианта без экранирования — для удобного копирования."""
    idx = int(callback.data.rsplit('_', 1)[1]) - 1
    data = await state.get_data()
    variants: List[Dict[str, str]] = data.get('variants') or []
    if idx < 0 or idx >= len(variants):
    ...
    # Шлём без parse_mode, чтобы Telegram показал его моноширинно
    # и кнопка «Копировать» работала без лишних тегов.
...
async def ai_generator_regen(callback: CallbackQuery, state: FSMContext):
    ...
    preview_lines = [f"{emoji('AI')} <b>Новые 3 варианта:</b>", ""]
    for i, v in enumerate(variants, 1):
        preview_lines.append(
            f"<b>Вариант {i}.</b> {escape(v['title'])}"
        )
