...
async def get_client_for_account(account_id: int) -> Optional[TelegramClient]:
    ...
    try:
        ...
        logger.error(f"Error connecting client for account {account_id}: {ex}")
        return None
...
async def get_chats_from_client(client: TelegramClient, limit: int = 200) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Error getting chats: {ex}")
    return chats

async def send_message_to_chat(client: TelegramClient, chat_id: str, text: str, media_paths: List[str] = None):
    try:
        ...
        logger.warning(f"Flood wait for {ex.seconds} seconds")
        await asyncio.sleep(ex.seconds + 1)
        return await send_message_to_chat(client, chat_id, text, media_paths)
    except Exception as ex:
...
async def execute_broadcast(broadcast_id: int, user_id: int):
    ...
    try:
        ...
        if not broadcast_stop_flags.get(broadcast_id, False):
            async with db_pool.acquire() as conn:
                await conn.execute(
...
{emoji('APPS')} <b>Функции</b> — рассылка и автоответчик
...
async def process_phone(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{emoji('CROSS')} Неверный номер телефона.")
        await state.clear()
    except PhoneNumberBannedError:
        await message.answer(f"{emoji('CROSS')} Этот номер заблокирован.")
        await state.clear()
    except FloodWaitError as ex:
        await message.answer(f"{emoji('CROSS')} Слишком много попыток. Подождите {ex.seconds} секунд.")
        await state.clear()
    except Exception as ex:
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            # Код истёк, отправляем новый автоматически
            try:
                ...
                await message.answer(
                    f"{emoji('CLOCK')} Код истёк. Новый код отправлен!\n\n"
                    f"Введите новый код из Telegram:",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(
                            text="Отмена",
                            callback_data="account_manager",
                ...
                await message.answer(f"{emoji('CROSS')} Подождите {ex.seconds} секунд перед повторной отправкой.")
                await state.clear()
                return
            except Exception as retry_ex:
                await message.answer(f"{emoji('CROSS')} Ошибка при повторной отправке кода: {str(retry_ex)}")
                await state.clear()
                return
        except PhoneCodeInvalidError:
            await message.answer(
                f"{emoji('CROSS')} Неверный код. Попробуйте снова:",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(
                        text="Отмена",
                        callback_data="account_manager",
...
{emoji('GEAR')} Режим: {'Одновременный' if data['mode'] == 'simultaneous' else 'Рандомный'}
    ...
    # Отправляем чистое сообщение пользователя
    if media_paths:
        if len(media_paths) == 1 and os.path.exists(media_paths[0]):
            ...
            valid_media = [p for p in media_paths if os.path.exists(p)]
            if valid_media:
                await message.answer(f"{emoji('MEDIA')} Прикреплено медиа: {len(valid_media)} файлов")
    else:
...
async def start_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            f"{emoji('CROSS')} Ошибка при запуске рассылки: {str(ex)}",
            reply_markup=get_functions_keyboard()
        )
...
async def my_broadcasts(callback: CallbackQuery):
    ...
    for bc in broadcasts[:10]:
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"{status_text} ID:{bc['id']} | {progress} | {bc['status']}",
...
{emoji('CALENDAR')} Создана: {bc['created_at'].strftime('%d.%m.%Y %H:%M')}
    """
...
{emoji('TAG')} Триггер: {data['trigger']}
...
{emoji('TAG')} Триггер: {responder['trigger']}
{emoji('WRITE')} Ответ: {responder['response_text'][:100]}...
{emoji('CLOCK')} Создан: {responder['created_at'].strftime('%d.%m.%Y %H:%M')}
    """
...
async def process_admin_broadcast(message: Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            logger.error(f"Failed to send admin broadcast to {user['user_id']}: {ex}")
