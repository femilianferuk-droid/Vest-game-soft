...
async def execute_dm_broadcast(task_id: int, account_id: int, user_id: int, usernames: List[str], message_text: str, delay: int, media_paths: List[str] = None):
    """Выполняет рассылку в ЛС по юзернеймам"""
    client = await get_client_for_account(account_id)
    if not client:
        return False
...
def get_dm_broadcast_control_keyboard(task_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(
            text="Остановить",
            callback_data=f"stop_dm_{task_id}",
            style='danger',
...
async def start_dm_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_dm_broadcast_control_keyboard(task_id)
    )
    
    task = asyncio.create_task(
        execute_dm_broadcast(
            task_id,
            data['account_id'],
            user_id,
            data['usernames'],
...
async def stop_dm_broadcast(callback: CallbackQuery):
    task_id = int(callback.data.split("_")[2])
    dm_broadcast_stop_flags[task_id] = True
    
    if task_id in dm_broadcast_tasks:
        dm_broadcast_tasks[task_id].cancel()
