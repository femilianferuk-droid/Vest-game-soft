...
# Pro-подписка: несколько сроков на выбор (см. PRO_PLANS).
...
PRO_PLANS: Dict[str, Dict[str, Any]] = {
    "pro_1d": {
        "code": "pro_1d", "days": 1, "rub": 5, "usd": "0.08",
        "title": "1 день", "badge": "тест",
    },
    "pro_7d": {
        "code": "pro_7d", "days": 7, "rub": 15, "usd": "0.23",
        "title": "7 дней", "badge": "",
    },
    "pro_30d": {
        "code": "pro_30d", "days": 30, "rub": 40, "usd": "0.60",
        "title": "30 дней", "badge": "популярный",
    },
    "pro_90d": {
        "code": "pro_90d", "days": 90, "rub": 100, "usd": "1.50",
        "title": "90 дней", "badge": "выгодно",
    },
    "pro_365d": {
        "code": "pro_365d", "days": 365, "rub": 350, "usd": "5.25",
        "title": "365 дней", "badge": "максимум",
    },
}
...
def pro_plan_button_text(plan: Dict[str, Any]) -> str:
    """Подпись кнопки выбора срока: «7 дней — 15₽»."""
    badge = f" · {plan['badge']}" if plan.get("badge") else ""
    return f"{plan['title']} — {plan['rub']}₽{badge}"
...
def pro_plans_text() -> str:
    ...
    for code in PRO_PLAN_ORDER:
        ...
        lines.append(
            f"  • <b>{plan['title']}</b> — {plan['rub']}₽{extra}{badge}"
        )
    return "\n".join(lines)
...
async def cryptopay_create_invoice(
    ...
    if payload == "wallet_topup":
        ...
        description = f"Vest Game Soft — Pro подписка ({plan['title']})"
    params = {
...
async def platega_create_transaction(
    ...
    Для Pro сумма в рублях берётся из PRO_PLANS по payload, если не задана явно.
    """
    try:
        bot_me = await bot.get_me()
...
async def buy_pro(callback: CallbackQuery):
    ...
    is_active_pro = sub.get("tier") == "pro"
    ...
    header = (
        f"{emoji('MONEY_SEND')} <b>Продление Pro</b>\n\n"
