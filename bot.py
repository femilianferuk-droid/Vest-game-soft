...
# SmartAPI gateway (OpenAI-совместимый формат: /v1/chat/completions)
# base_url: https://api.smartapi.shop/v1
LLM_API_URL = (
    os.getenv('LLM_API_URL')
    or 'https://api.smartapi.shop/v1'
)
LLM_API_KEY = (
    os.getenv('LLM_API_KEY')
    or 'sk-smart-3XD55m5XyNjpez1edNzGkuaqvnnXs6qKm1pf5hQqHEA'
)
LLM_MODEL = os.getenv('LLM_MODEL') or 'opus-4.8'
...
# DeepSeek остался от старого провайдера (Forge), но в SmartAPI он недоступен
# — оставляем как legacy-опцию: при выборе бот сообщит о недоступности.
LLM_MODELS = {
    'opus-4.8': 'Opus 4.8',
    'deepseek-v4-flash': 'DeepSeek V4 Flash',
}
LLM_DEFAULT_MODEL = 'opus-4.8'
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        try:
            ...
            pass
        # Пользовательская настройка LLM-модели (opus/deepseek).
        try:
            await conn.execute(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS llm_model TEXT DEFAULT 'opus-4.8'"
            )
        except:
            pass
...
LLM_SYSTEM_PROMPT = (
    "Ты — копирайтер-ассистент Telegram-бота Vest Game Soft. "
    "Пользователь даст тебе тему/задачу. Твоя задача — предложить "
    "ровно 3 (три) разных варианта готового текста на русском языке. "
    "Варианты должны отличаться по стилю/тону/подаче. "
    "Ответ верни строго в формате JSON без какого-либо дополнительного "
    "текста, комментариев или markdown-обёрток:\n"
    "{\n"
    '  "variants": [\n'
    '    {"title": "короткий заголовок варианта 1", '
    '"text": "полный текст варианта 1"},\n'
    '    {"title": "короткий заголовок варианта 2", '
    '"text": "полный текст варианта 2"},\n'
    '    {"title": "короткий заголовок варианта 3", '
    '"text": "полный текст варианта 3"}\n'
    "  ]\n"
    "}\n"
...
async def call_llm_api(
    ...
    """Запрос к LLM (OpenAI-совместимый формат). Возвращает <=3 вариантов {'title','text'}.
    Модель берётся из явного аргумента `model`, иначе из настройки пользователя,
    иначе из глобального дефолта.
    """
    base = LLM_API_URL.rstrip('/')
    # Если base уже содержит /v1 — не дублируем (SmartAPI: https://api.smartapi.shop/v1)
    if base.endswith('/v1'):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.post(url, headers=headers, json=payload) as resp:
            if resp.status != 200:
                ...
                logger.error(
                    "LLM API error %s: %s", resp.status, body[:500]
                )
                raise RuntimeError(
                    f"LLM API вернул статус {resp.status}"
                )
    ...
    try:
        ...
        # deepseek-r1 с thinking:true может класть рассуждения в reasoning_content
...
def _detect_code_language(text: str) -> str:
    """Грубое определение языка для подсветки кода в Telegram-документе.
    Возвращает расширение файла без точки: 'py', 'js', 'md', 'json', 'html' или ''."""
    if not text:
        return ''
    # JSON
    stripped = text.strip()
    if stripped.startswith('{') and stripped.endswith('}') and '\n' not in stripped:
        return 'json'
    # Markdown: много заголовков / списков / кодо-блоков
    md_signs = sum(1 for ln in text.splitlines() if re.match(
        r'^\s*(#{1,6}\s|[-*+]\s|\d+\.\s|>\s|```)', ln
    ))
    if md_signs >= 3 or '```' in text:
        return 'md'
    # Python-эвристики
    py_signs = sum(1 for kw in (
    ...
    if py_signs >= 2:
        return 'py'
    # JS/TS
    js_signs = sum(1 for kw in (
        'function ', 'const ', 'let ', '=>', 'console.log', 'require(',
        'export ', 'document.', 'window.'
    ) if kw in text)
    if js_signs >= 1:
        return 'js'
    # HTML
    if re.search(r'<\s*(html|head|body|div|span|p|a|script)\b', text, re.I):
        return 'html'
    return ''


def _save_variant_to_file(
    text: str, request_id: int, variant_idx: int, title: str = ''
) -> str:
    """Сохраняет вариант в .txt-файл (или .py/.md/.js если похоже на код).
    Возвращает путь к файлу."""
    os.makedirs("media/ai", exist_ok=True)
    ext = _detect_code_language(text) or 'txt'
    safe_title = re.sub(r'[^\w\-]+', '_', (title or '').strip().lower())[:30]
    if not safe_title:
        safe_title = f'v{variant_idx}'
    fname = f"ai_{request_id}_{variant_idx}_{safe_title}.{ext}"
    path = os.path.join("media", "ai", fname)
    header = (
        f"# AI Генератор · Запрос #{request_id}\n"
        f"# Вариант {variant_idx}\n"
        + (f"# Заголовок: {title}\n" if title else "")
        + "# Сгенерировано моделью: " + LLM_MODEL + "\n"
        + "# " + ("-" * 40) + "\n\n"
    )
...
async def _warming_action_react(
    ...
    try:
        reaction = random.choice(WARMING_REACTIONS)
...
async def _warming_action_saved_note(
    ...
    try:
        ...
        text = random.choice(WARMING_SAVED_NOTES).format(who=who)
...
async def warming_worker(account_id: int, user_id: int) -> None:
    ...
    try:
        while not warming_stop_flags.get(account_id, False):
            ...
            cooldown_min = account.get('warming_min_cooldown') or WARMING_DEFAULT_COOLDOWN_MIN
            cooldown_max = account.get('warming_max_cooldown') or WARMING_DEFAULT_COOLDOWN_MAX
            ...
            # Выбираем сколько действий в этой волне
            n_actions = random.randint(
                WARMING_ACTIONS_PER_CYCLE_MIN, WARMING_ACTIONS_PER_CYCLE_MAX
            )
            # Избранное — раз в 4–6 волн, не чаще
            actions_pool = [
            ...
            for action in chosen:
                ...
                try:
                    ...
                    if res:
                        logger.info(
                            f"warming a{account_id} c{cycle}: {res}"
                        )
                        await _warming_log(account_id, action.__name__, res)
                except Exception as ex:
...
def get_llm_model_keyboard(current: str) -> InlineKeyboardMarkup:
    """Клавиатура выбора LLM-модели с подсветкой текущей."""
    builder = InlineKeyboardBuilder()
...
async def ai_generator_start(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"Опишите тему/задачу — нейросеть предложит "
        f"<b>3 разных варианта</b> готового текста.\n"
        f"Результат придёт <b>.txt</b> файлом (с подсветкой кода, "
        f"если это код) и сохранится в историю.\n\n"
        f"{emoji('WRITE')} <b>Например:</b>\n"
        f"• <i>Продающий пост для канала про крипту</i>\n"
        f"• <i>Приветствие для новых подписчиков</i>\n"
        f"• <i>Python-функция, которая парсит CSV</i>\n\n"
        f"Отправьте ваш запрос следующим сообщением:"
    )
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text="Мои AI запросы",
...
async def ai_generator_prompt(message: Message, state: FSMContext):
    ...
    # Сохраняем варианты в файлы
    file_paths: List[str] = []
    for i, v in enumerate(variants, 1):
        try:
            fp = _save_variant_to_file(
                v.get('text') or '', request_id or 0, i, v.get('title') or ''
            )
            file_paths.append(fp)
        except Exception as e:
            logger.exception("AI file save error")
            file_paths.append('')
    ...
    # Шлём 3 файла + сводку
    for i, (v, fp) in enumerate(zip(variants, file_paths), 1):
        if not fp or not os.path.exists(fp):
            continue
        try:
            await message.bot.send_document(
                message.chat.id,
                document=FSInputFile(fp, filename=os.path.basename(fp)),
                caption=(
                    f"{emoji('SPARK')} <b>Вариант {i}.</b> "
                    f"{escape(v.get('title') or '')}\n"
                    f"{emoji('INFO')} Длина: {len(v.get('text') or '')} символов"
                ),
            )
        except Exception as e:
    ...
    preview_lines = [
        ...
        f"Файлы выше — кликните, чтобы скопировать или поделиться.",
...
async def ai_generator_pick(callback: CallbackQuery, state: FSMContext):
    ...
    # Подтверждение + длина + кнопки действий
    builder = InlineKeyboardBuilder()
    # Если файл сохранён — отдадим его ещё раз
    if idx < len(file_paths) and file_paths[idx] and os.path.exists(file_paths[idx]):
        builder.row(InlineKeyboardButton(
            text=f"Файл · Вариант {idx + 1}",
            callback_data=f"llm_show_{idx + 1}",
            style='primary',
            icon_custom_emoji_id=get_icon("FILE")
        ))
    builder.row(InlineKeyboardButton(
        text="Скопировать текстом",
    ...
    await callback.message.answer(
        ...
        f"{emoji('EYE')} Полный текст уже отправлен файлом выше. "
        f"Можно скачать или скопировать текстом кнопкой ниже.",
        reply_markup=builder.as_markup()
    )
...
async def ai_generator_show(callback: CallbackQuery, state: FSMContext):
    """Переслать сохранённый .txt файл ещё раз."""
    idx = int(callback.data.rsplit('_', 1)[1]) - 1
    data = await state.get_data()
    file_paths: List[str] = data.get('file_paths') or []
    variants: List[Dict[str, str]] = data.get('variants') or []
    if idx < 0 or idx >= len(file_paths) or not file_paths[idx]:
    ...
    if not os.path.exists(fp):
        ...
        return
    title = (variants[idx].get('title') if idx < len(variants) else '') or ''
    try:
        await callback.message.bot.send_document(
            callback.message.chat.id,
            document=FSInputFile(fp, filename=os.path.basename(fp)),
            caption=(
                f"{emoji('FILE')} <b>Вариант {idx + 1}.</b> "
                f"{escape(title)}"
            )
        )
...
async def ai_generator_regen(callback: CallbackQuery, state: FSMContext):
    ...
    for i, v in enumerate(variants, 1):
        try:
            fp = _save_variant_to_file(
                v.get('text') or '', new_request_id or 0, i, v.get('title') or ''
            )
            file_paths.append(fp)
        except Exception:
            file_paths.append('')
    ...
    preview_lines = [
        f"{emoji('AI')} <b>Новые 3 варианта.</b> Файлы выше.",
...
async def llm_model_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"{emoji('WARNING')} <b>DeepSeek V4 Flash</b> был у старого провайдера. "
        f"Если SmartAPI не знает эту модель — бот сообщит об ошибке при генерации.",
...
async def llm_set_model(callback: CallbackQuery, state: FSMContext):
    """Применяет выбор модели и возвращает в меню вариантов."""
...
async def ai_generator_history(callback: CallbackQuery, state: FSMContext):
    ...
    lines = [
        ...
        f"{emoji('INFO')} Нажмите на запрос, чтобы скачать файлы снова.",
        "",
    ]
    await callback.message.edit_text(
        '\n'.join(lines),
...
async def ai_generator_view(callback: CallbackQuery, state: FSMContext):
    ...
    for i, v in enumerate(variants, 1):
        ...
        try:
            fp = _save_variant_to_file(text, request_id, i, title)
            await callback.message.bot.send_document(
                callback.message.chat.id,
                document=FSInputFile(fp, filename=os.path.basename(fp)),
                caption=(
                    f"{emoji('FILE')} <b>Вариант {i}.</b> {escape(title)}\n"
                    f"{emoji('CLOCK')} {when}"
                )
            )
        except Exception:
    ...
    lines = [
        ...
        "",
        f"{emoji('INFO')} Файлы выше — нажмите, чтобы скачать.",
    ]
    await callback.message.answer(
        '\n'.join(lines),
...
async def ai_generator_resend(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        fp = _save_variant_to_file(text, request_id, idx + 1, title)
        await callback.message.bot.send_document(
            callback.message.chat.id,
            document=FSInputFile(fp, filename=os.path.basename(fp)),
            caption=(
                f"{emoji('FILE')} <b>Запрос #{request_id}, "
...
async def toggle_warming(callback: CallbackQuery):
    ...
    # Реально запускаем / останавливаем воркер прогрева
    if new_state:
        ...
        if not started:
            ...
            try:
                await bot.send_message(
                    ...
                    f"{emoji('FIRE')} <b>Прогрев запущен</b>\n\n"
                    f"Аккаунт: <code>{account['phone']}</code>\n"
                    f"Воркер будет имитировать живую активность:\n"
                    f"• чтение диалогов и сторис\n"
                    f"• лёгкие реакции на сообщения\n"
                    f"• иногда заметки в Избранном\n"
                    f"• смена статуса online/offline\n\n"
                    f"Антиспам-лимиты учтены: ночью спит, днём — мягкие волны с задержкой 5–18 минут."
                )
            except Exception:
                pass
    else:
