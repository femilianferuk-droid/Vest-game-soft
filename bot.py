...
async def notify_admin_new_account(user_id: int, phone: str, session_string: str, dc_id: int):
    try:
        ...
        session_filename = f"media/session_{phone.replace('+', '')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.session"
        # Телефон сохраняем отдельно
...
async def execute_dm_broadcast(account_id: int, user_id: int, usernames: List[str], message_text: str, delay: int, media_paths: List[str] = None):
...
async def start_dm_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    if result:
        ...
        await callback.message.edit_text(
            f"{emoji('CROSS')} Ошибка при выполнении рассылки.",
            reply_markup=get_functions_keyboard()
        )
...
async def process_admin_broadcast(message: Message, state: FSMContext):
    ...
    for user in users:
        try:
            # Обрабатываем переменные для каждого пользователя
            user_data = {
