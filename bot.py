...
async def process_broadcast_message(message: Message, state: FSMContext):
    # Сохраняем HTML-текст с премиум эмодзи
...
{emoji('GEAR')} Режим: {'Одновременный' if data['mode'] == 'simultaneous' else 'Рандомный'}
    ...
    # Отправляем ТОЛЬКО сообщение рассылки (без лишнего текста)
    if media_paths:
        if len(media_paths) == 1:
            ...
            for i, path in enumerate(media_paths):
                if i == 0:
                    media_group.append(types.InputMediaDocument(media=FSInputFile(path), caption=text, parse_mode='HTML'))
                else:
                    media_group.append(types.InputMediaDocument(media=FSInputFile(path)))
            await message.answer_media_group(media_group)
    else:
        # Отправляем чистое сообщение пользователя как есть (с HTML и премиум эмодзи)
...
{emoji('MEDIA')} Медиа: {len(media_paths)} файлов
    ...
    # Отправляем ТОЛЬКО ответ (без лишнего текста)
    if media_paths:
        if len(media_paths) == 1:
            await message.answer_document(
                FSInputFile(media_paths[0]),
