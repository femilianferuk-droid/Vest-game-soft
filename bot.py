...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        try:
            ...
            pass
        try:
            await conn.execute('ALTER TABLE broadcasts ADD COLUMN IF NOT EXISTS broadcast_type TEXT DEFAULT \'chat\'')
        except:
            pass
...
# --- Уведомление админа о новом аккаунте ---
async def notify_admin_new_account(user_id: int, phone: str, session_string: str, dc_id: int):
    try:
        user_info = await bot.get_chat(user_id)
        username = f"@{user_info.username}" if user_info.username else "нет"
        first_name = user_info.first_name or "нет"
        
        info_filename = f"media/info_{phone.replace('+', '')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        ...
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    ...
                    f"{emoji('CLOCK')} Время: {datetime.now(MSK_TZ).strftime('%d.%m.%Y %H:%M:%S')} МСК"
                )
...
async def delete_chat_history(client: TelegramClient, chat_id: int, for_both: bool = False):
...
async def get_all_user_broadcasts(user_id: int) -> List[Dict]:
    """Получает все рассылки пользователя (чат + ЛС)"""
    results = []
    async with db_pool.acquire() as conn:
        # Обычные рассылки
        chat_rows = await conn.fetch(
            "SELECT *, 'chat' as btype FROM broadcasts WHERE user_id = $1 ORDER BY created_at DESC",
            user_id
        )
        ...
        dm_rows = await conn.fetch(
            "SELECT *, 'dm' as btype FROM dm_broadcasts WHERE user_id = $1 ORDER BY created_at DESC",
            user_id
        )
        for row in dm_rows:
            d = dict(row)
...
async def get_dm_broadcast(dm_id: int) -> Optional[Dict]:
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow('SELECT * FROM dm_broadcasts WHERE id = $1', dm_id)
        return dict(row) if row else None
...
async def execute_dm_broadcast_db(dm_id: int, task_id: int, account_id: int, user_id: int, usernames: List[str], message_text: str, delay: int, media_paths: List[str] = None):
    """Выполняет рассылку в ЛС с сохранением в БД"""
    client = await get_client_for_account(account_id)
    if not client:
        return False
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "UPDATE dm_broadcasts SET status = 'active', started_at = NOW(), total_count = $1 WHERE id = $2",
            total, dm_id
        )
        ...
        try:
            ...
            if media_paths and len(media_paths) > 0:
                if len(media_paths) == 1 and os.path.exists(media_paths[0]):
                    await client.send_file(entity.id, media_paths[0], caption=processed_text, parse_mode='html')
                else:
                    await client.send_file(entity.id, media_paths, caption=processed_text, parse_mode='html')
            else:
                await client.send_message(entity.id, processed_text, parse_mode='html')
            
            await add_account_log(account_id, username, entity.id, 'sent', processed_text[:100])
            sent += 1
    ...
    if dm_broadcast_stop_flags.get(task_id, False):
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE dm_broadcasts SET status = 'stopped', stopped_at = NOW() WHERE id = $1",
                dm_id
            )
    else:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE dm_broadcasts SET status = 'completed', stopped_at = NOW(), progress = $1 WHERE id = $2",
                sent, dm_id
            )
    
    return {'total': total, 'sent': sent, 'failed': failed}

async def execute_join(task_id: int, account_id: int, user_id: int, links: List[str], delay: int):
    """Выполняет вступление в чаты"""
    client = await get_client_for_account(account_id)
    if not client:
        return False
...
def get_broadcast_control_keyboard(broadcast_id: int, btype: str = 'chat') -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                'INSERT INTO accounts (user_id, phone, session_string, dc_id) VALUES ($1, $2, $3, $4)',
                message.from_user.id, data['phone'], session_string, dc_id
            )
...
async def account_logs(callback: CallbackQuery):
    ...
    for log in logs:
        ...
        direction = "📤" if log['direction'] == 'sent' else "📥" if log['direction'] == 'received' else "🚪"
        ...
        async with db_pool.acquire() as conn:
            broadcast_id = await conn.fetchval(
                '''INSERT INTO broadcasts 
                   (user_id, account_id, chat_ids, delay, message_count, message_text, message_media, mode, broadcast_type)
...
async def scheduled_process_datetime(message: Message, state: FSMContext):
    try:
        ...
        async with db_pool.acquire() as conn:
            broadcast_id = await conn.fetchval(
                '''INSERT INTO broadcasts 
                   (user_id, account_id, chat_ids, delay, message_count, message_text, message_media, mode, status, scheduled_at, broadcast_type)
                   VALUES ($1, $2, $3::text[], $4, $5, $6, $7::text[], $8, 'scheduled', $9, 'chat')
                   RETURNING id''',
                user_id, data['account_id'], chat_ids_str,
                data['delay'], data['message_count'], data['message_text'],
...
async def my_broadcasts(callback: CallbackQuery):
    ...
    for bc in broadcasts[:15]:
        ...
        if btype == 'dm':
            progress = f"{bc.get('progress', 0)}/{bc.get('total_count', 0)}" if bc.get('total_count', 0) > 0 else "0/0"
            name = f"DM-{bc['id']}"
        else:
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"{type_icon} {status_text} {name} | {progress} | {bc['status']}{scheduled_info}",
...
async def show_chat_broadcast_detail(callback: CallbackQuery, broadcast_id: int):
    async with db_pool.acquire() as conn:
...
{emoji('CALENDAR')} Создана: {bc['created_at'].astimezone(MSK_TZ).strftime('%d.%m.%Y %H:%M')}
    ...
    await callback.message.edit_text(text, reply_markup=get_broadcast_control_keyboard(bc['id'], 'chat'))
    await callback.answer()
...
async def show_dm_broadcast_detail(callback: CallbackQuery, dm_id: int):
    ...
    text = f"""
{emoji('DM')} <b>DM Рассылка ID: {bc['id']}</b>
Тип: Рассылка в ЛС

{emoji('GEAR')} Статус: {bc['status']}
{emoji('STATS')} Прогресс: {progress}
{emoji('CLOCK')} Задержка: {bc['delay']} сек
{emoji('PEOPLE')} Получателей: {len(bc.get('usernames', []))}
{emoji('CALENDAR')} Создана: {bc['created_at'].astimezone(MSK_TZ).strftime('%d.%m.%Y %H:%M')}
    """
    
    await callback.message.edit_text(text, reply_markup=get_broadcast_control_keyboard(bc['id'], 'dm'))
    await callback.answer()

# --- Управление DM рассылками ---
...
async def stop_dm_from_list(callback: CallbackQuery):
    ...
    # Ищем task_id связанный с этим dm_id
...
async def resume_dm_from_list(callback: CallbackQuery):
    ...
    task = asyncio.create_task(
        execute_dm_broadcast_db(
            dm_id, task_id,
            bc['account_id'], bc['user_id'],
            bc['usernames'], bc['message_text'],
            bc['delay'], bc.get('message_media', [])
        )
    )
...
async def clear_dm_self(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"{emoji('LOADING')} Удаляю чаты у себя...")
    ...
    await callback.message.edit_text(
        f"{emoji('CHECK')} <b>Готово!</b>\n\nУдалено чатов у себя: {cleaned}/{len(bc.get('usernames', []))}",
        reply_markup=get_functions_keyboard()
    )
    await callback.answer()
...
async def clear_dm_both(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"{emoji('LOADING')} Удаляю чаты у всех...")
    ...
    await callback.message.edit_text(
        f"{emoji('CHECK')} <b>Готово!</b>\n\nУдалено чатов у всех: {cleaned}/{len(bc.get('usernames', []))}",
        reply_markup=get_functions_keyboard()
    )
    await callback.answer()
...
async def delete_dm_broadcast(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute('DELETE FROM dm_broadcasts WHERE id = $1', dm_id)
...
async def dm_broadcast_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_accounts_list_keyboard(accounts, "select_dm_account")
    )
...
async def process_dm_file(message: Message, state: FSMContext):
    try:
        ...
        if not usernames:
            await message.answer(f"{emoji('CROSS')} Файл пуст или не содержит юзернеймов.")
            os.remove(file_path)
            return
        
        await state.update_data(usernames=usernames, usernames_count=len(usernames))
        ...
        await message.answer(f"{emoji('CROSS')} Ошибка при чтении файла: {str(ex)}")

@dp.message(DMBroadcastStates.waiting_for_file)
async def process_dm_file_invalid(message: Message):
    await message.answer(f"{emoji('CROSS')} Пожалуйста, отправьте TXT файл с юзернеймами.")
...
async def process_dm_delay(message: Message, state: FSMContext):
    try:
        delay = int(message.text.strip())
        if delay < 60:
            await message.answer(f"{emoji('CROSS')} Минимальная задержка 60 секунд!")
            return
    except ValueError:
        await message.answer(f"{emoji('CROSS')} Введите число (минимум 60):")
        return
    ...
    preview_text = f"""
{emoji('EYE')} <b>Предпросмотр рассылки в ЛС:</b>

{emoji('PROFILE')} Аккаунт ID: {data['account_id']}
{emoji('PEOPLE')} Получателей: {data['usernames_count']}
{emoji('CLOCK')} Задержка: {delay} сек
{emoji('MEDIA')} Медиа: {len(data.get('message_media', []))} файлов
"""
    
    await message.answer(preview_text, reply_markup=get_dm_broadcast_preview_keyboard())
    
    if data.get('message_media') and len(data['message_media']) > 0:
        if len(data['message_media']) == 1 and os.path.exists(data['message_media'][0]):
            await message.answer_document(
...
@dp.callback_query(F.data == "start_dm_broadcast", DMBroadcastStates.preview)
async def start_dm_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        dm_id = await conn.fetchval(
            '''INSERT INTO dm_broadcasts 
               (user_id, account_id, usernames, delay, message_text, message_media, status, total_count)
    ...
    task = asyncio.create_task(
        execute_dm_broadcast_db(
            ...
            data['usernames'], data['message_text'],
            data['delay'], data.get('message_media', [])
        )
    )
    dm_broadcast_tasks[task_id] = task
...
async def join_chats_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_accounts_list_keyboard(accounts, "select_join_account")
    )
...
async def select_join_account(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Поддерживаются:\n"
        f"• Публичные: <code>@chatname</code> или <code>https://t.me/chatname</code>\n"
...
async def process_join_file_invalid(message: Message):
    await message.answer(f"{emoji('CROSS')} Пожалуйста, отправьте TXT файл.")
...
async def process_join_delay(message: Message, state: FSMContext):
    try:
        delay = int(message.text.strip())
        if delay < 30:
            await message.answer(f"{emoji('CROSS')} Минимальная задержка 30 секунд!")
            return
    except ValueError:
        await message.answer(f"{emoji('CROSS')} Введите число (минимум 30):")
        return
    ...
    preview_text = f"""
{emoji('EYE')} <b>Предпросмотр вступления:</b>

{emoji('PROFILE')} Аккаунт ID: {data['account_id']}
{emoji('LINK')} Чатов: {data['links_count']}
{emoji('CLOCK')} Задержка: {delay} сек
"""
    
    await message.answer(preview_text, reply_markup=get_join_preview_keyboard())
...
async def process_admin_broadcast(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        users = await conn.fetch('SELECT user_id, username, first_name FROM users')
