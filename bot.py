...
EMOJI = {
    ...
    "SUPPORT": ("🎧", "6039486778597970865"),  # Используем эмодзи наушников для поддержки
}
...
async def get_chats_from_client(client: TelegramClient, limit: int = 200) -> List[Dict]:
    ...
    async for dialog in client.iter_dialogs(limit=limit):
        if dialog.is_user or dialog.is_group or dialog.is_channel:
            ...
            chat_info = {
                'id': str(dialog.id),  # Конвертируем в строку сразу
                'name': dialog.name,
...
async def send_message_to_chat(client: TelegramClient, chat_id: str, text: str, media_paths: List[str] = None):
    try:
        # Конвертируем chat_id обратно в int для Telethon
...
async def process_broadcast_message(message: Message, state: FSMContext):
    ...
    # Отправляем предпросмотр ВМЕСТО текущего сообщения пользователя
    preview_text = f"""
...
{emoji('GEAR')} Режим: {'Одновременный' if data['mode'] == 'simultaneous' else 'Рандомный'}
    ...
    # Отправляем пример сообщения отдельно
    if media_paths:
        # Отправляем медиа с подписью
        if len(media_paths) == 1:
            await message.answer_document(
                types.FSInputFile(media_paths[0]),
                caption=f"{emoji('WRITE')} <b>Сообщение для рассылки:</b>\n\n{text[:500]}{'...' if len(text) > 500 else ''}",
                reply_markup=get_broadcast_preview_keyboard()
            )
        else:
            await message.answer(
                f"{emoji('WRITE')} <b>Сообщение для рассылки:</b>\n\n{text[:500]}{'...' if len(text) > 500 else ''}\n\n{emoji('MEDIA')} Прикреплено медиа: {len(media_paths)} файлов",
                reply_markup=get_broadcast_preview_keyboard()
            )
    else:
        await message.answer(
...
async def start_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    # Конвертируем chat_ids в строки для PostgreSQL
