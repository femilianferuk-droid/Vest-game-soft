...
async def get_user_accounts(user_id: int) -> List[Dict]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            'SELECT id, phone, is_active, warming_enabled FROM accounts WHERE user_id = $1',
            user_id
        )
...
async def check_scheduled_broadcasts():
    """Проверяет и запускает отложенные рассылки"""
    while True:
        try:
...
def get_scheduled_broadcast_preview_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(
            text="Запланировать",
...
# --- Отложенная рассылка ---
@dp.callback_query(F.data == "chats_done")
async def scheduled_chats_done(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if not data.get('is_scheduled'):
        return await chats_done(callback, state)
    ...
    await state.set_state(ScheduledBroadcastStates.waiting_for_delay)
    await callback.answer()

# Добавляем обработчики для scheduled состояний (delay, count, message, datetime)
...
async def process_parsing_chat(message: Message, state: FSMContext):
    ...
    client = await get_client_for_account(account_id)
    if not client:
        await message.answer(f"{emoji('CROSS')} Не удалось подключиться к аккаунту.")
        await state.clear()
        return
    
    await message.answer(f"{emoji('LOADING')} Собираю участников из <code>{chat_username}</code>...")
    ...
    try:
        ...
        # Сохраняем в файл
        filename = f"parsed_{chat_username.replace('@', '')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        filepath = f"media/{filename}"
        
        with open(filepath, 'w', encoding='utf-8') as f:
            for user in users:
                f.write(f"{user['user_id']}|{user['username']}|{user['first_name']}\n")
...
async def on_startup():
    ...
    # Запускаем проверку отложенных рассылок
