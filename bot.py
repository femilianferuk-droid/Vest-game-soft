...
async def call_llm_api(user_prompt: str) -> List[Dict[str, str]]:
    """Запрос к LLM. Возвращает список из <=3 вариантов {'title','text'}."""
    url = LLM_API_URL.rstrip('/') + '/v1/messages'
    headers = {
    ...
    # Anthropic-совместимый ответ: content[].text
    content = ''
    try:
    ...
    if not content and isinstance(data, dict):
        # fallback для OpenAI-совместимого ответа
        try:
