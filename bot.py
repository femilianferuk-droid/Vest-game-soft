...
def e(name: str) -> str:
    """Форматирует премиум эмодзи для текста"""
    if name in EMOJI:
...
def get_icon(name: str) -> str:
    """Возвращает ID эмодзи для кнопок"""
    if name in EMOJI:
        return EMOJI[name][1]
    return None
...
async def cmd_start(message: Message):
    ...
    welcome_text = f"""
{e('SMILE')} <b>Добро пожаловать в Vest Game Soft!</b>

{e('BOT')} Я помогу вам управлять аккаунтами и делать рассылки.

{e('PEOPLE')} <b>Менеджер аккаунтов</b> — добавление и управление аккаунтами
{e('APPS')} <b>Функции</b> — рассылка и автоответчик
...
async def cmd_admin(message: Message):
    ...
    admin_text = f"""
{e('BOT')} <b>Админ-панель</b>

{e('PEOPLE')} Пользователей: <b>{stats['total_users']}</b>
{e('PROFILE')} Аккаунтов: <b>{stats['total_accounts']}</b>
{e('MEGAPHONE')} Всего рассылок: <b>{stats['total_broadcasts']}</b>
{e('PLAY')} Активных: <b>{stats['active_broadcasts']}</b>
    """
...
async def account_manager(callback: CallbackQuery):
    await callback.message.edit_text(
        f"{e('PEOPLE')} <b>Менеджер аккаунтов</b>\n\nВыберите действие:",
...
async def add_account(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        f"{e('PHONE')} <b>Добавление аккаунта</b>\n\n"
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if not re.match(r'^\+\d{10,15}$', phone):
        await message.answer(
            f"{e('CROSS')} Неверный формат номера. Попробуйте снова.\n"
    ...
    try:
        ...
        await message.answer(
            f"{e('CHECK')} Код подтверждения отправлен!\n\n"
        ...
        await message.answer(f"{e('CROSS')} Ошибка: {str(e)}")
        await state.clear()
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            await message.answer(
                f"{e('LOCK_CLOSED')} Введите пароль 2FA:",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(
                        text="Отмена",
                        callback_data="account_manager",
        ...
        await message.answer(
            f"{e('CHECK')} Аккаунт успешно добавлен!",
            reply_markup=get_account_manager_keyboard()
        )
        await state.clear()
...
async def my_accounts(callback: CallbackQuery):
    ...
    if not accounts:
        await callback.message.edit_text(
            f"{e('INFO')} У вас пока нет аккаунтов.\n\n"
            f"Нажмите 'Добавить аккаунт' чтобы добавить новый.",
            reply_markup=get_account_manager_keyboard()
        )
    else:
        await callback.message.edit_text(
            f"{e('PEOPLE')} <b>Ваши аккаунты:</b>\n\n"
...
async def manage_account(callback: CallbackQuery):
    ...
    text = f"""
{e('PROFILE')} <b>Аккаунт:</b>
{e('PHONE')} Телефон: <code>{account['phone']}</code>
{e('EYE')} Статус: {'✅ Активен' if account['is_active'] else '❌ Неактивен'}
{e('CLOCK')} Создан: {account['created_at'].strftime('%d.%m.%Y %H:%M')}
    """
...
async def delete_account_handler(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{e('CHECK')} Аккаунт успешно удален!",
        reply_markup=get_account_manager_keyboard()
    )
    await callback.answer()
...
async def select_mode(callback: CallbackQuery, state: FSMContext):
    ...
    if not accounts:
        await callback.message.edit_text(
            f"{e('CROSS')} У вас нет аккаунтов. Сначала добавьте аккаунт.",
            reply_markup=get_functions_keyboard()
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        f"{e('PROFILE')} <b>Выберите аккаунт для рассылки:</b>",
...
async def select_broadcast_account(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('PEOPLE')} <b>Выберите чаты для рассылки</b> (макс. 200)\n"
...
async def chats_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('CLOCK')} <b>Введите задержку между сообщениями</b>\n\n"
...
async def process_delay(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"{e('CROSS')} Введите число от 10 до 300000:"
        )
        return
    ...
    await message.answer(
        f"{e('MAIL')} <b>Введите количество сообщений в каждый чат</b>\n\n"
        f"От 1 до 200000:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="Назад",
...
async def process_count(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"{e('CROSS')} Введите число от 1 до 200000:"
        )
        return
    ...
    await message.answer(
        f"{e('WRITE')} <b>Введите сообщение для рассылки:</b>\n\n"
...
async def process_broadcast_message(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    preview_text = f"""
{e('EYE')} <b>Предпросмотр рассылки:</b>

{e('PROFILE')} Аккаунт ID: {data['account_id']}
{e('PEOPLE')} Чатов: {len(data['selected_chats'])}
{e('CLOCK')} Задержка: {data['delay']} сек
{e('MAIL')} Сообщений в чат: {data['message_count']}
{e('GEAR')} Режим: {'Одновременный' if data['mode'] == 'simultaneous' else 'Рандомный'}

{e('WRITE')} <b>Сообщение:</b>
{text[:200]}{'...' if len(text) > 200 else ''}

{e('MEDIA')} Медиа: {len(media_paths)} файлов
    """
...
async def start_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('PLAY')} <b>Рассылка запущена!</b>\n\n"
        f"ID: {broadcast_id}\n"
...
async def stop_broadcast(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{e('STOP')} <b>Рассылка остановлена!</b>\n\nID: {broadcast_id}",
        reply_markup=get_broadcast_control_keyboard(broadcast_id)
    )
    await callback.answer()
...
async def resume_broadcast(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{e('PLAY')} <b>Рассылка возобновлена!</b>\n\nID: {broadcast_id}",
        reply_markup=get_broadcast_control_keyboard(broadcast_id)
    )
    await callback.answer()
...
async def my_broadcasts(callback: CallbackQuery):
    ...
    if not broadcasts:
        await callback.message.edit_text(
            f"{e('INFO')} У вас пока нет рассылок.",
            reply_markup=get_functions_keyboard()
        )
        await callback.answer()
        return
...
async def show_broadcast(callback: CallbackQuery):
    ...
    text = f"""
{e('CHART')} <b>Рассылка ID: {bc['id']}</b>

{e('GEAR')} Статус: {bc['status']}
{e('STATS')} Прогресс: {progress}
{e('CLOCK')} Задержка: {bc['delay']} сек
{e('MAIL')} Сообщений в чат: {bc['message_count']}
{e('PEOPLE')} Чатов: {len(bc['chat_ids'])}
{e('CALENDAR')} Создана: {bc['created_at'].strftime('%d.%m.%Y %H:%M')}
    """
...
async def auto_responder(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('PROFILE')} <b>Выберите аккаунт для автоответчика:</b>",
...
async def select_responder_account(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('WRITE')} <b>Введите слово-триггер:</b>\n\n"
...
async def process_trigger(message: Message, state: FSMContext):
    ...
    if not trigger:
        await message.answer(f"{e('CROSS')} Введите слово-триггер или '-'")
        return
    
    await state.update_data(trigger=trigger)
    
    await message.answer(
        f"{e('WRITE')} <b>Введите ответ:</b>\n\n"
        f"Поддерживается HTML и премиум эмодзи.\n"
...
async def process_auto_response(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    preview_text = f"""
{e('EYE')} <b>Предпросмотр автоответчика:</b>

{e('TAG')} Триггер: {data['trigger']}
{e('WRITE')} Ответ:
{text[:200]}{'...' if len(text) > 200 else ''}
...
async def create_auto_responder(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('CHECK')} <b>Автоответчик создан и запущен!</b>\n\n"
        f"ID: {responder_id}\n"
...
async def my_auto_responders(callback: CallbackQuery):
    ...
    if not responders:
        await callback.message.edit_text(
            f"{e('INFO')} У вас пока нет автоответчиков.",
            reply_markup=get_functions_keyboard()
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        f"{e('BELL')} <b>Мои автоответчики:</b>",
        reply_markup=builder.as_markup()
    )
    await callback.answer()
...
async def show_responder(callback: CallbackQuery):
    ...
    text = f"""
{e('BELL')} <b>Автоответчик ID: {responder['id']}</b>

{e('EYE')} Статус: {'✅ Активен' if responder['is_active'] else '❌ Остановлен'}
{e('TAG')} Триггер: {responder['trigger']}
{e('WRITE')} Ответ: {responder['response_text'][:100]}...
{e('CLOCK')} Создан: {responder['created_at'].strftime('%d.%m.%Y %H:%M')}
    """
...
async def admin_broadcast_all(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{e('MEGAPHONE')} <b>Рассылка всем пользователям</b>\n\n"
