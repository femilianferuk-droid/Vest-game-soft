...
# Forge gateway (OpenAI-совместимый формат: /v1/chat/completions)
LLM_API_URL = (
    os.getenv('LLM_API_URL')
    or 'https://forge-gateway-api.fly.dev'
)
LLM_API_KEY = (
...
async def call_llm_api(user_prompt: str) -> List[Dict[str, str]]:
    """Запрос к LLM (OpenAI-совместимый формат). Возвращает <=3 вариантов {'title','text'}."""
    url = LLM_API_URL.rstrip('/') + '/v1/chat/completions'
    headers = {
...
async def ai_generator_prompt(message: Message, state: FSMContext):
    ...
    thinking = await message.answer(
        ...
        f"{emoji('BOT')} Модель: <code>{escape(LLM_MODEL)}</code>\n"
        f"{emoji('WRITE')} Запрос: <i>{escape(user_prompt[:200])}</i>"
    )

    try:
        variants = await call_llm_api(user_prompt)
    ...
    # Сохраняем в БД
    try:
        request_id = await save_ai_request(message.from_user.id, user_prompt, variants)
    except Exception as e:
