...
def _acct_ar_status_text(settings: Dict[str, Any], phone: str) -> str:
...
def _acct_ar_main_keyboard(account_id: int, mode: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
async def account_ai_responder_worker(account_id: int, user_id: int):
    ...
    try:
        ...
        if not await client.is_user_authorized():
            logger.warning(
                f"acct_ar: account {account_id} не авторизован, воркер выходит"
            )
            await client.disconnect()
            return
        ...
        async def _handler(event):
            ...
            if not text:
                return  # стикеры/фото пока не обрабатываем
            ...
            try:
                ...
                # История по этому собеседнику + новое сообщение
                history = list(
                ...
                try:
                    await client.send_message(
                        chat_id,
                        f"⚠️ ИИ-автоответчик: ошибка LLM: {escape(str(ex))}",
                    )
                except Exception:
                    pass
                return
            ...
            try:
                if len(answer) <= 4000:
                    ...
                    for i in range(0, len(answer), 4000):
                        await client.send_message(chat_id, answer[i:i + 4000])
            except Exception as ex:
                logger.exception("acct_ar: не удалось отправить ответ: %s", ex)

            # Лог аккаунта
            try:
                await add_account_log(
                    account_id, sender_name, chat_id, 'received', text[:100]
                )
                await add_account_log(
                    account_id, sender_name, chat_id, 'sent',
                    answer[:100]
                )
            except Exception:
                pass
...
async def cb_acct_ar_home(callback: CallbackQuery, state: FSMContext):
    ...
    if not account or account['user_id'] != callback.from_user.id:
        ...
        return
    settings = await acct_ar_get(account_id)
    text = _acct_ar_status_text(settings, account['phone'])
    try:
        await callback.message.edit_text(
            text, reply_markup=_acct_ar_main_keyboard(
                account_id, settings['mode']
            )
        )
    except Exception:
        await callback.message.answer(
...
async def process_acct_ar_system(message: Message, state: FSMContext):
    ...
    account = await get_account(account_id)
    settings = await acct_ar_get(account_id)
    text_out = _acct_ar_status_text(settings, account['phone'])
    await message.answer(
        "✅ Личность ИИ обновлена.\n\n" + text_out,
        reply_markup=_acct_ar_main_keyboard(account_id, settings['mode']),
    )
...
async def cb_acct_ar_model_set(callback: CallbackQuery):
    ...
    await stop_account_ai_responder(account_id, callback.from_user.id)
    settings = await acct_ar_get(account_id)
    if settings['mode'] == ACCT_AR_MODE_AI:
        await start_account_ai_responder(account_id, callback.from_user.id)
    text = _acct_ar_status_text(settings, account['phone'])
    try:
        await callback.message.edit_text(
            text, reply_markup=_acct_ar_main_keyboard(
...
async def cmd_cancel_acct_ar(message: Message, state: FSMContext):
    """Локальный /cancel для AccountAIResponderStates — не ломает чужой /cancel."""
